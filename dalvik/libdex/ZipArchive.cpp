        ...
        ZipArchive* pArchive, off64_t fileLength, size_t readAmount, u1* scanBuf)
{
	...
	//TEMP_FAILURE_RETRY 于忽略系统中断造成的错误。常用于系统调用。
	//zip的官方文档https://pkware.cachefly.net/webdocs/APPNOTE/APPNOTE-6.2.0.txt
	//获取local file header signature     ，长度为4字节，值为：(0x04034b50)
    ssize_t actual = TEMP_FAILURE_RETRY(read(pArchive->mFd, scanBuf, sizeof(int32_t)));
    if (actual != (ssize_t) sizeof(int32_t)) {
        ALOGI("couldn't read first signature from zip archive: %s", strerror(errno));
        return false;
    }
	//获取signature值并判断是否与(0x04034b50)相等
    ...
    //寻找end of central dir signature(0x06054b50)
    //readAmount = 65535 + 22
    //i = 65535 假设刚好末尾就是EOCD且file comment=0
    //现在scanBuf[i]=0x06054b50的第一个字节 格式是小端字节所以刚好是50，然后在判断全部值
    int i;
    ...
    ZipArchive* pArchive)
{
    ...
    //也就是 End of Central Directory 的最大长度，因为 Zip file comment 最大长度为 64KB
    size_t readAmount = kMaxEOCDSearch; //65535 + 22
...
static int parseZipArchive(ZipArchive* pArchive)
{
    int result = -1;
	//核心目录开始地址
    const u1* cdPtr = (const u1*)pArchive->mDirectoryMap.addr;
	//核心目录长度
