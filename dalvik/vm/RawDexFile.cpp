...
static int verifyMagicAndGetAdler32(int fd, u4 *adler32)
{
     ...
     * four bytes of checksum. //8字节魔数4字节checksum
     */
    u1 headerStart[12];
    ...
    RawDexFile** ppRawDexFile, bool isBootstrap)
{
    ...
    if (newFile) {
        ...
        if (result) {
            startWhen = dvmGetRelativeTimeUsec();
			 //将dex文件拷贝到优化dex文件中（也就是DexOptHeader后面，
            //因为现在optFd指向的是DexOptHeader末尾，
     ...
     */
0        ALOGI("Unable to map cached %s", fileName);
        goto bail;
    }
