...
{	
    ...
    if (pid == 0) {
		...
		//execFile = "/system/bin/dexopt";
        strcat(execFile, kDexOptBin);

        /*
         * Create arg vector. //创建参数向量
         */
        int curArg = 0;
		//kUseValgrind = 0，不执行if?
        if (kUseValgrind) {
		...
		//kUseValgrind = 0 ，!0为真; kFixedArgCount = 10,curArg = 10 （++了10次）
        assert(((!kUseValgrind && curArg == kFixedArgCount) ||
               ((kUseValgrind && curArg == kFixedArgCount+kValgrindArgCount))));
		//将gDvm.bootClassPath中的fileName加入argv,应该是为了比较是否有加载与系统同名的类（注释有说）
        ClassPathEntry* cpe;
    ...
    u4 crc, bool expectVerify, bool expectOpt)
{
	...
	//验证依赖库大小是否正确，新创建dex流程中并未看到optHdr.depsLength的赋值
	//那优化头又是在哪儿赋值的呢？猜想可能是在执行优化命令后续有赋值
	//看了下optHdr.depsLength的调用在optMain.cpp的main函数中有调用，后面再看到底哪儿赋值的
    ...
    ptr = depData;
	//获取修改时间，这里需要先看下odex的格式，我是看源码遇到问题了再去看，先看下会好很多。
	//Dependencies前四位存的是dex的modWhen 小端（Little-Endian）字节
    val = read4LE(&ptr);
